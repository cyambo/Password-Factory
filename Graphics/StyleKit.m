...
//  Password Factory
//
//  Created by Cristiana Yambo on 11/10/17.
...
+ (void)drawStoredType
{
    [StyleKit drawStoredTypeWithFrame: NSMakeRect(0, 0, 250, 250) resizing: StyleKitResizingBehaviorAspectFit];
}

+ (void)drawStoredTypeWithFrame: (NSRect)targetFrame resizing: (StyleKitResizingBehavior)resizing
{
    //// General Declarations
    CGContextRef context = (CGContextRef)NSGraphicsContext.currentContext.graphicsPort;
...
{
    if (_imageOfStoredType)
        return _imageOfStoredType;

    _imageOfStoredType = [NSImage imageWithSize: NSMakeSize(250, 250) flipped: NO drawingHandler: ^(__unused NSRect dstRect)
    {
        [StyleKit drawStoredType];
